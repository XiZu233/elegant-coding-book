...
学完了基础语法和 AI 工具操作，现在是检验成果的时候了。本章提供两个完整的实战项目，目标是让你体验"描述需求 → AI生成代码 → 运行验证 → 迭代优化"的完整闭环。你不需要安装任何第三方库，只用 Python 自带的工具就能完成。每个案例都包含完整的对话流程——你可以直接复制提示词，跟着做。

实战案例的价值不在于"做出了什么工具"，而在于你学会了如何与 AI 协作解决真实问题。这是 AI 时代编程最核心的能力。
...
完成这两个实战项目后，你应该已经体会到了 AI 编程的效率。但当你想进一步扩展项目时，可能会遇到一些"知其然不知其所以然"的困惑：为什么需要环境变量？路径中的 `/` 和 `\\` 有什么区别？`pip` 和 `conda` 是什么关系？

第06章将系统补充这些"编程基础设施"知识——HTTP/JSON、文件系统、依赖管理、Docker、终端与进程。这些知识不是让你成为专家，而是让你能**看懂 AI 生成的代码在做什么**，并在出问题时知道从哪里排查。
